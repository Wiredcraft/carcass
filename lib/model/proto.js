...
// TODO:
// * Rebuild `remove()`, `save()`, `update()`, `error()`.
// * Rebuild `isNew()`.

var noop = function() {};

// Default store method. Requires a storage.
exports.store = function(method, item, fn) {
    if (!this.storage) return fn(error('no storage'));
    var op = noop, storage = this.storage;
    switch (method) {
    case 'del':
        op = storage.del || noop;
        break;
    case 'put':
        ...
        break;
    case 'post':
        op = storage.post || this.storage.put || noop;
        break;
    case 'get':
        op = storage.get || noop;
        break;
